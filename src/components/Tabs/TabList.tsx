...
const TabList: FC<RTTabsListProps> = ({ children, className, style }) => {
    ...
    if (!Array.isArray(children)) {
        finalChildren = cloneElement(children as React.ReactElement<any>, {
            index: 0,
        });
    } else {
        finalChildren = Children.map(children, (child: ReactElement, index) =>
            cloneElement(child, {
                index,
            }),
        );
