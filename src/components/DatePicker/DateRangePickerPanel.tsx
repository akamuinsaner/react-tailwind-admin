...
const DatePickerPanelHeader = ({
    ...
    const prevMonth = () => {
        setCurrent({
            ...
            year:
                current[index].month === 1
                    ? current[index].year - 1
                    : current[index].year,
        });
    };
    const afterMonth = () => {
    ...
    const preIconClassNames = twMerge(
        styles.panel.header.icon,
        classNames({
            [styles.panel.header.disabledIcon]:
                index === 1 &&
                (current[1].year < current[0].year ||
                    (current[1].year == current[0].year &&
                        current[1].month <= current[0].month)),
        }),
    );

    const afterIconClassNames = twMerge(
        styles.panel.header.icon,
        classNames({
            [styles.panel.header.disabledIcon]:
                index === 0 &&
                (current[0].year > current[1].year ||
                    (current[0].year == current[1].year &&
                        current[0].month >= current[1].month)),
        }),
    );
    return (
        <div
        ...
        >
            ...
            <span className={styles.panel.header.display}>
                <span onClick={setYearMode}>
                    {MonthMap[current[index].month]}
                </span>
                <span onClick={setDecadeMode}>{current[index].year}</span>
            </span>
            <ChevronRightIcon
                onClick={afterMonth}
                className={afterIconClassNames}
            />
            <ChevronDoubleRightIcon
                onClick={afterYear}
                className={afterIconClassNames}
...
const DatePickerPanelBody = ({
    ...
    const itemClassName = (item: any) => {
        ...
        const filterByCompare =
            index === 0
                ? dayjs(item.date).isAfter(value[1])
                : dayjs(item.date).isBefore(value[0]);
        return twMerge(
            styles.panel.body.item,
            classNames({
                [styles.panel.body.notInMonth]: !item.inMonth,
                [styles.panel.body.isToday]: dayjs(item.date).isToday(),
                [styles.panel.body.selected]: dayjs(item.date).isSame(
                    value[index],
                ),
                [styles.panel.body.outOfRange]:
                    filterByLimit || filterByCompare,
            }),
        );
    };
    return (
        <div
            className={styles.panel.body.base}
            onClick={e => {
                e.stopPropagation();
                e.nativeEvent.stopImmediatePropagation();
            }}
...
const DatePickerDecadeMode = ({
    ...
    const itemClassName = (item: any) => {
        ...
        const filterByCompare =
            curIndex === 0
                ? item.year > current[1].year
                : item.year < current[0].year;
        return twMerge(
            styles.panel.year.item,
            classNames({
                [styles.panel.year.notIn]: !item.inDecades,
                [styles.panel.year.selected]:
                    current[curIndex].year === item.year,
                [styles.panel.body.outOfRange]:
                    filterByLimit || filterByCompare,
            }),
        );
...
const DatePickerCenturyMode = ({
    ...
    const itemClassName = (item: any) => {
        ...
        const filterByCompare =
            curIndex === 0
                ? item.start > current[1].year
                : item.end < current[0].year;
        return twMerge(
            styles.panel.year.item,
            classNames({
                [styles.panel.year.notIn]: !item.inCentury,
                [styles.panel.year.selected]:
                    current[curIndex].year > item.start &&
                    current[curIndex].year < item.end,
                [styles.panel.body.outOfRange]:
                    filterByLimit || filterByCompare,
            }),
        );
...
const DateRangePickerPanel = ({
    ...
    useEffect(() => {
        ...
        if (value?.[0])
            current[0] = {
                year: dayjs(value[0]).year(),
                month: dayjs(value[0]).month() + 1,
            };
        else current[0] = { year: dayjs().year(), month: dayjs().month() + 1 };
        if (value?.[1])
            current[1] = {
                year: dayjs(value[1]).year(),
                month: dayjs(value[1]).month() + 1,
            };
